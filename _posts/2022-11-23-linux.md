...
cd .. : 상위 디렉토리로 이동. 예를 들어 현재 위치가 /user/jtaewu라면 /user로 이동
...
### 1) cp [복사할 파일명] [붙여넣을 파일명] - 파일이나 디렉토리 복사
...
### 2) mv [이동할 파일] [이동될 위치] - 파일이나 디렉토리 이동
...
### 1) type [명령어명] - 지정된 명령어의 타입 (쉘 내장인지 외부 유래인지 앨리어스인지)을 확인
...
### 4) apropos [검색어] - 검색어와 관련있는 명령어의 사용법 출력
...
### 7) alias [별명]='명령어' - 명령어를 간단한 이름으로 사용할 수 있도록 별명 설정
